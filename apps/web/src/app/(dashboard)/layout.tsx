          ...
          {NAV_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                ...
                {!collapsed && isActive && <ChevronRight className="w-3 h-3 ml-auto text-indigo-500/50" />}
              </Link>
            );
          })}
        </nav>
      ...
      <main className={`flex-1 ${collapsed ? 'ml-16' : 'ml-56'} transition-all duration-200`}>
        ...
        <header className="h-14 border-b border-zinc-900 bg-[#09090b]/80 backdrop-blur-sm flex items-center justify-between px-6 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-2.5">
            <span className="px-2 py-0.5 rounded-md bg-indigo-500/8 border border-indigo-500/15 text-indigo-400 text-[11px] font-medium">
              {user?.plan || 'FREE'}
            </span>
            <div className="w-7 h-7 rounded-full bg-indigo-600/15 border border-indigo-500/20 flex items-center justify-center text-indigo-400 text-xs font-semibold">
              {user?.name?.[0]?.toUpperCase() || 'U'}
            </div>
          </div>
        </header>
